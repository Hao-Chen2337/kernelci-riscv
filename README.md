...
with a file per command - and the reason for that is in
[src/DESIGN.md](src/DESIGN.md).  **The rewrite won, and the old tree is deleted**
...
`lib/i18n.py`; `python3 lib/i18n.py --check lib/gui.py` is that file's test (missing keys,
keys nothing uses, rows left untranslated), and `--map` says which line of `gui.py` reads
each one.
...
with the measurement behind each claim.  `src/GUI.md` §12 is the same round written as
contract.  The acceptance harness is `docs/gui-rework/tools/accept.py` - sixteen checks
...
| the two kselftest suites run | `kselftest-riscv` 10/10 pass, `kselftest-kvm` 9 pass / 3 skip (the skips need real hardware) |
...
| 我想… | 读 | 它在说什么 |
|---|---|---|
| 把新树跑起来 | 本文件的「The new tree: how to run it」 | 七条命令，每条都真跑过 |
| 知道哪些话是被验证过的、哪些还没有 | 本文件的「What is verified」+「Not done yet」 | 证据与缺口，分开写 |
| 看懂接口的形状 | [`include/kci.hpp`](include/kci.hpp)（总览）→ `include/kci/*.hpp`（按层） | 只有声明和注释，`g++ -std=c++17` 能过 |
| 知道为什么长这样、什么不能改 | [`src/DESIGN.md`](src/DESIGN.md) | 诊断、取舍、逐字节契约、按时间排的实测记录 |
| 知道页面怎么设计的 | [`src/GUI.md`](src/GUI.md) | 远端/本地/对应三样东西、九个页面、控件→命令对照 |
| **把页面用起来**（起哪台、切 `api`、出错那句什么意思、已知边界） | [`src/GUI-OPS.md`](src/GUI-OPS.md) | 操作者的一页纸：两个旋钮、三条流程、什么时候需要重启、怎么自己复核 |
| 看所有者最初的手写草稿 | [`src/sketch/`](src/sketch/) | 一字未改，留作对照 |
| **操作这个部署**（部署、起栈、worker、prune、verify） | [`docs/RUNBOOK.md`](docs/RUNBOOK.md) | 部署与运行手册：环境、setup、stack、worker、prune、verify |
| 知道上游卡在哪、项目边界在哪 | `docs/UPSTREAM-BLOCKERS.md`、`docs/STATUS-AND-LIMITS.md`、`docs/HANDOVER.md` | 关于外部世界的事实，日期是测量日期 |
| 看旧树（已删除）内部怎么分的层 | `docs/ARCHITECTURE.md`、`docs/PROJECT-MAP.md` | 旧树的地图，只当历史读 |
| 翻历史 | `docs/archive/`（冻结） | 别当现状读 |

**两份文档集的分界**：`docs/` 是**这个部署**的运行手册加上关于上游与旧树的事实
（旧树 `scripts/` + `run.sh` 已于 2026-09-20 删除）；`src/` + `include/` 讲的是
...
| Path | Contents |
|---|---|
| `lib/` | the new middle layer: `kbuild`/`build`/`kjob`/`job`/`out` + `poller`, `judge`, `runner`, `sink`, `api`, `layout`, `config`, `tests`, `re`, `drift`, `gui`, `run`, `i18n` |
| `*.py` (root) | the new entry points, one file each |
| `include/kci/*.hpp` | the interface, as C++ declarations - read `include/kci.hpp` first; the Python matches it name for name |
| `src/DESIGN.md` | why the rewrite, what it deletes, what must not change, and the measurements |
| `src/GUI.md` | the page's design |
| `src/sketch/` | the owner's original hand-written sketch, kept for comparison |
| `deploy/` | **the deployment half**: `setup.sh` (clones + patches + this deployment's local configuration + the YAML check), `stack.sh [--seed]` (the local full stack: compose, artifact server, real callback, official scheduler), `stop.sh`, `instance-init.sh`, `seed.sh`, `net-preflight.sh`, `render-local-config.py`.  It owns no runtime decision - the root entries do - and it reads and writes `var/` like the rest of this tree |
| `docs/RUNBOOK.md` | how to operate this deployment (setup, stack, worker, prune, verify) |
| `config/` | PR1 config patch + tuxlava/bullseye/nginx patches |
| `kernelci-*/` | upstream clones (gitignored, created by `deploy/setup.sh`) |
| `var/` | the workspace (gitignored): fetched builds, bakes, consoles, activities, deployment state, and the ledger - history, never pruned.  `$KCI_WORK_DIR` moves it.  The old tree's `work/` is deleted; its data is archived in `/home/hao/kci-work-backup-*.tar.gz` |
...
`STATUS-AND-LIMITS.md`, `UPSTREAM-BLOCKERS.md` and `HANDOVER.md` describe the
*shipping* tree, and their numbers are measurements from the dates they name,
not standing claims.  Two of them are worth repeating here because they are the
deliverable rather than the tooling: the test profile (`config/pr1-config.patch`,
