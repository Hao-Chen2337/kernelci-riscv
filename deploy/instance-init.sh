#!/usr/bin/env bash
# Local KernelCI instance init: generate the runtime config that
# `deploy/setup.sh` deliberately leaves out. Idempotent; safe to re-run.
#
...
# Keep in sync with deploy/stack.sh: the compose file publishes
...
# matching deploy/stack.sh's readiness probe.
...
# ---------------------------------------------------------------- 1) .env --
if [ ! -d "$API_DIR" ]; then
  echo "  !! skip kernelci-api/.env: kernelci-api/ not cloned yet (run deploy/setup.sh)"
...
if [ ! -d "$PIPE_DIR" ] || [ ! -d "$API_DIR" ]; then
  echo "  !! skip SSH keypair: kernelci-pipeline/ or kernelci-api/ not cloned yet (run deploy/setup.sh)"
...
if [ "$SKIP_TOKEN" -eq 1 ]; then
  ...
  echo "  !! skip KCI_API_TOKEN: kernelci-api/ not cloned yet (run deploy/setup.sh)"
elif [ ! -f "$API_ENV" ]; then
...
[ "$QUIET" -eq 1 ] || echo
[ "$QUIET" -eq 1 ] || echo "next step: bash $ROOT/deploy/stack.sh"
