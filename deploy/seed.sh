#!/usr/bin/env bash
# The seed half of deploy/stack.sh, which SOURCES this file (never runs
...
# The record `python3 provision.py` writes (`var/state/served.json`) as shell
# assignments: one python3 call, shell-quoted, so the seed never parses JSON and
# never keeps a list of keys in step with a second file.  The old tree sourced
# `var/state/served.json`, which was the same facts written a second time by the same
# command - two copies, one of which could be edited.
#
# Every scalar the record carries is printed (a list becomes one space-joined
# value: the tag list), dictionaries and booleans are skipped, and a key the seed
# does not know is simply an unused variable.
served_assignments() {   # file
  [ -f "$1" ] || return 0
  python3 - "$1" <<'PYSEED'
...
for key, value in sorted((data or {}).items()):
    ...
    if value is None or isinstance(value, (dict, bool)):
        continue
    print(f"KCI_SERVED_{key.upper()}={shlex.quote(str(value))}")
PYSEED
}

resolve_seed_inputs() {
  # CONSISTENCY RULE: var/serve/Image, the modules and kselftest these URLs name
  # must be the SAME kbuild (modprobe matches by kernel release), so read
  # var/state/served.json - the record `python3 provision.py` wrote; the SEED_*
  # defaults below only cover a never-provisioned stack.  The guest rootfs is not
  # part of this rule: it is the lab's own image (lib/tests.ROOTFS_URL), baked per
  # run into the bake cache.
  if [ -f "$ROOT/var/state/served.json" ]; then
    eval "$(served_assignments "$ROOT/var/state/served.json")"
    if [ -n "${KCI_SERVED_BUILD_DIR:-}" ]; then
      echo "    seeding from the provisioned build: $KCI_SERVED_BUILD_DIR"
      SEED_MODULES_URL="${SEED_MODULES_URL:-$KCI_SERVED_BUILD_DIR/modules.tar.xz}"
      SEED_KSELFTEST_URL="${SEED_KSELFTEST_URL:-$KCI_SERVED_BUILD_DIR/kselftest.tar.xz}"
      SEED_CONFIG_URL="${SEED_CONFIG_URL:-$KCI_SERVED_BUILD_DIR/.config}"
    fi
    ...
    SEED_COMMIT="${SEED_COMMIT:-${KCI_SERVED_COMMIT:-}}"
    SEED_DESCRIBE="${SEED_DESCRIBE:-${KCI_SERVED_DESCRIBE:-}}"
    SEED_TREE="${SEED_TREE:-${KCI_SERVED_TREE:-}}"
    SEED_BRANCH="${SEED_BRANCH:-${KCI_SERVED_BRANCH:-}}"
    SEED_TREE_URL="${SEED_TREE_URL:-${KCI_SERVED_TREE_URL:-}}"
    SEED_VERSION="${SEED_VERSION:-${KCI_SERVED_VERSION:-}}"
    SEED_PATCHLEVEL="${SEED_PATCHLEVEL:-${KCI_SERVED_PATCHLEVEL:-}}"
    SEED_TAGS="${SEED_TAGS:-${KCI_SERVED_TAGS:-}}"
  fi
  ...
  if [ -z "${KCI_SERVED_COMMIT:-}" ]; then
    ...
    echo "     $SEED_DESCRIBE, which is a placeholder, not the kernel in var/serve/Image."
    echo "     Fix: python3 provision.py (records it), or set SEED_COMMIT/SEED_DESCRIBE."
  fi
  if [ -z "$SEED_TAGS" ] && [ -n "${KCI_SERVED_COMMIT:-}" ]; then
...
PY
  ...
  die "refusing to seed tree '$SEED_TREE': the pull-labs-riscv runtime does not accept it
    ...
    var/state/served.json records this deployment's build as tree '${KCI_SERVED_TREE:-?}',
    ...
    Two honest ways out:
      * keep the artifacts and label the seed with an accepted tree (the node's tree
        then disagrees with the kernel that boots - recorded in var/state/seed.env):
          SEED_TREE=${allowed_trees%% *} deploy/stack.sh --seed
      * or let this runtime accept the build's tree, in the scheduler's own config:
          add '${KCI_SERVED_TREE:-<tree>}' to runtimes.pull-labs-riscv.rules.tree in
...
# tree=$SEED_TREE while the artifacts come from the tree=$KCI_SERVED_TREE build,
# and that difference is invisible in the API.
write_seed_provenance() {
  mkdir -p "$ROOT/var/state"
  {
    echo "# Written by deploy/stack.sh --seed.  What the seeded nodes claim, and"
    ...
    echo "KCI_SEED_BUILD_TREE=${KCI_SERVED_TREE:-}"
    echo "KCI_SEED_BUILD_DIR=${KCI_SERVED_BUILD_DIR:-}"
  ...
  if [ "$1" = 1 ]; then
    ...
    echo "     boot come from the tree=${KCI_SERVED_TREE:-?} build (${KCI_SERVED_BUILD_DIR:-no build.env})"
    echo "     recorded in var/state/seed.env (KCI_SEED_TREE_MISMATCH=1)."
  else
    echo "    seed provenance recorded in var/state/seed.env"
  fi
}
...
seed_prepare() {
  ...
  if [ -n "${KCI_SERVED_TREE:-}" ] && [ "$KCI_SERVED_TREE" != "$SEED_TREE" ]; then
...
seed_stack() {
  ...
  [ -f "$SERVE_DIR/Image" ] || die "seed needs $SERVE_DIR/Image (run python3 provision.py, or drop one there)"
...
try:
  ...
  if curl -s -m 30 -o /dev/null -X POST -H "Authorization: Bearer $TOKEN" \
    ...
    echo "    (seed provenance: var/state/seed.env; tree label $SEED_TREE, build tree ${KCI_SERVED_TREE:-none recorded})"
  else
