...
Verification performed live against api.github.com on 2026-09-09 (unauthenticated REST, HTTP 200 for every fetch). All raw JSON saved under 'docs/upstream-evidence/' (list at the end).
...
- Top-level .gitignore ignores kernelci-*/ checkouts; runs/ and artifacts/ are ignored so 'git add -A' stays clean.
...
- Recommendation before handover (now applied): .gitignore covers 'runs/' and 'artifacts/'; never run 'git add -A' at the top level.
...
## Saved files (all fetched JSON, in docs/upstream-evidence/)
