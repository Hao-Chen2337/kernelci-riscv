...
#
"""Host-port probes: is the port a service is about to bind already taken, and by whom.

Binding is the only honest test - a listener that answers nothing still owns the
port - and `deploy/stack.sh` probes through this module's command line, so there is
one implementation of "is this port free" and not two that can differ.

This is `kcilib/core/ports.py` moved into the tree that outlives it: the stack's
deployment half still needs the probe, and the old tree's copy would have gone with
`kcilib`.  The behaviour is unchanged, including the two things it got right the
hard way - a port published by *our own* compose project is not a conflict (the
partly-up stack), and a refusal always names the holder.
"""
...
# partly-up stack); the default is what deploy/stack.sh builds as PROJECT.
...
def require_port_free(port, who, override_var, project=None,
    ...
    *host* is the address the probe binds and is NOT decoration: probing
    ...
    another interface through, which is why deploy/stack.sh passes host="0.0.0.0".
    """
    ...
    raise SystemExit(
        ...
        "  (the KCI_*_PORT list is at the top of deploy/stack.sh)"
    )


def main(argv=None):
    """The probe as a command line, for deploy/stack.sh.
