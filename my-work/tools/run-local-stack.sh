...
SERVE_DIR="$MYWORK/evidence/worker-out/kvm-loop/serve"
SERVE_PORT=8999
CB_PORT=8003
...
if curl -s -m 3 -o /dev/null http://127.0.0.1:8001/latest/; then
  ...
  echo "-> starting: docker compose -p kapi2 up -d api redis storage ssh"
  (cd "$API_DIR" && docker compose -p kapi2 up -d api redis storage ssh >/dev/null) || die "compose failed"
...
if curl -s -m 3 -o /dev/null "http://127.0.0.1:$CB_PORT/"; then
  ...
  (cd "$PIPE_DIR/src" && KCI_SETTINGS="$MYWORK/config/local-callback.toml" KCI_API_TOKEN="$TOKEN" PYTHONPATH="$ROOT/kernelci-core" setsid nohup python3 -m uvicorn lava_callback:app --port $CB_PORT --host 0.0.0.0 >/tmp/cb$CB_PORT.log 2>&1 < /dev/null &)
  sleep 4
...
def merge(a, b):
    for k, v in b.items():
        if isinstance(v, dict) and isinstance(a.get(k), dict):
            ...
            a[k] = v
    return a
a = yaml.safe_load(open("$PIPE_DIR/config/pipeline.yaml"))
b = yaml.safe_load(open("$MYWORK/config/cb-config/pipeline.yaml"))
open(sys.argv[1], "w").write(yaml.safe_dump(merge(a, b), sort_keys=False))
PYEOF
  (cd "$KCFG" && KCI_SETTINGS="$MYWORK/config/local-callback.toml" KCI_API_TOKEN="$TOKEN" KCI_INSTANCE_CALLBACK="http://127.0.0.1:$CB_PORT" PYTHONPATH="$ROOT/kernelci-core" setsid nohup python3 "$PIPE_DIR/src/scheduler.py" --yaml-config "$KCFG/config" --settings "$MYWORK/config/local-callback.toml" loop --runtimes pull-labs-riscv --name local-full-stack --output /tmp/sched-output >/tmp/sched-local.log 2>&1 < /dev/null &)
  sleep 10
  ...
  curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" --data @- http://127.0.0.1:8001/latest/node <<EOF
{
  ...
  "data": {"arch": "riscv", "defconfig": "defconfig", "compiler": "gcc-14",
           "kernel_revision": {"tree": "riscv", "branch": "master",
...
if [ "${1:-}" = "--worker" ]; then
  ...
  PATH=/usr/local/sbin:/usr/sbin:$PATH PULL_LABS_CALLBACK_TOKEN=labtoken-callback python3 "$MYWORK/tools/lab/riscv_pull_worker.py" --api-url http://127.0.0.1:8001 --tuxrun-bin /home/hao/.local/bin/tuxrun --container-runtime docker --output-dir /tmp/official-loop-out --state-file /tmp/official-loop-state.json --poll-period 5 --max-timeout 1200
else
  echo 'next step (manual):'
  echo "  PULL_LABS_CALLBACK_TOKEN=labtoken-callback python3 $MYWORK/tools/lab/riscv_pull_worker.py --api-url http://127.0.0.1:8001 --tuxrun-bin /home/hao/.local/bin/tuxrun --container-runtime docker --poll-period 5"
fi
