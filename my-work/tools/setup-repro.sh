...
git -C kernelci-pipeline apply ../my-work/config/pr1-config.patch || \
  git -C kernelci-pipeline apply --3way ../my-work/config/pr1-config.patch || \
...
if grep -q 'archive.debian.org' kernelci-api/docker/ssh/Dockerfile 2>/dev/null; then
  ...
  git -C kernelci-api apply ../my-work/config/kernelci-api-bullseye-archive.patch \
...
echo
echo '完成。下一步:'
echo '  bash my-work/tools/run-local-stack.sh --seed   # 起 API+回调+官方调度器并派单'
