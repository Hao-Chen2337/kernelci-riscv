...
this repository actually runs (two run paths and one local table):
    ...
    model/      the objects a caller holds: Build cards, Jobs, Outcomes, the
                stack, the page and the ledger reader - the layer on top of all
                of the above, which nothing below may import back
...
Five programs use the library - and since 2026-09-19 they all use THIS package:
the interface layer that used to sit beside it (`scripts/kci/`) moved in as
`kcilib/model/`, because all five imported kcilib anyway, so it was a second
vocabulary rather than a face.  The programs: `scripts/riscv_pull_worker.py` (a resident
...
| `table/localrun.py` | `ran_tests()`, `todo()` | The views over the table ("what have I not run yet") | `ran_tests()`/`todo()` are pure reads - the ledger answers "did this machine run it", the API answers "what does upstream think", and only the ledger answers it offline. The execution shell that used to live here (`run_job()`/`outcome_from()`, wrapping `kcilib.run.jobrun.run_node`) was deleted on 2026-09-18: the model layer (`kcilib/model/jobs.py`, `Jobs`/`Job.run()`) had taken it over and what remained had no caller but a guard - and keeping it would now mean `table/` importing upward into `model/` |
