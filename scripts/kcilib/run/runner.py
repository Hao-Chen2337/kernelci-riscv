...
def build_tuxrun_argv(*, tuxrun_bin, runtime, device, kernel,
    ...
    of the flag disappearing silently.  Rationale: docs/docs/code-notes/W2c-kcilib.md.
    """
    argv = [
        tuxrun_bin,
        "--runtime",
...
def run_tuxrun(argv, timeout=None, log_path=None, *, cwd=None,
    ...
    *stream_separator* goes between the captured stdout and stderr, which are
    ...
    console and ``stderr`` is empty.  Rationale: docs/docs/code-notes/W2c-kcilib.md.
    """
    try:
        proc = subprocess.run(
            argv,
